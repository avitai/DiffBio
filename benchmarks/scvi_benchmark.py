...
Usage:
    python benchmarks/scvi_benchmark.py
"""
...
from diffbio.operators.singlecell.soft_clustering import SoftClusteringConfig, SoftKMeansClustering
...
def generate_synthetic_pbmc_data(
    ...
    # Each type has a distinct expression pattern
    ...
    # Build per-cell log-mean expression: type mean + batch shift + cell noise
    ...
    # dispersion parameter (larger = less overdispersion)
    dispersion = 5.0
...
def run_scvi_benchmark(
    ...
    # Latent silhouette (bio conservation): how well cell types separate
...
def save_results(result: ScVIBenchmarkResult, output_dir: Path) -> None:
    ...
    output_file = output_dir / f"scvi_benchmark_{timestamp}.json"
...
def main() -> None:
    """Main entry point for scVI benchmark."""
    print("=" * 60)
    ...
    results = run_scvi_benchmark(n_epochs=50)
    ...
    print(f"  Training time:      {results['training_time_ms']:.0f}ms")
