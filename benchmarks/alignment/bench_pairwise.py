...
class PairwiseBenchmark(DiffBioBenchmark):
    ...
    def _run_core(self) -> dict[str, Any]:
        ...
        operator = SmoothSmithWaterman(op_config, scoring_matrix=scoring_matrix)
        ...
        for family in families:
            ...
            ref_seqs = [(name, _strip_gaps(seq)) for name, seq in family["reference"]]
            ...
            for (name_a, seq_a), (name_b, seq_b) in pairs:
                ...
                encoded_a = onehot_encode_sequence(truncated_a, len(truncated_a))
                encoded_b = onehot_encode_sequence(truncated_b, len(truncated_b))
            ...
            avg_family = sum(family_scores) / len(family_scores) if family_scores else 0.0
            logger.info(
        ...
        n_finite = sum(1 for s in all_scores if np.isfinite(s))
        logger.info(
        ...
        sample_seqs = [
            _strip_gaps(seq)[:max_seq_length] for _, seq in first_family["reference"][:2]
        ]
        # Ensure we have at least 2 sequences for gradient check
        if len(sample_seqs) < 2:
            sample_seqs = [sample_seqs[0], sample_seqs[0]]

        sample_a = onehot_encode_sequence(sample_seqs[0], len(sample_seqs[0]))
        sample_b = onehot_encode_sequence(sample_seqs[1], len(sample_seqs[1]))
