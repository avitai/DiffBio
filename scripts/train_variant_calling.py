...
def evaluate(
    pipeline: nnx.Module,
    inputs: list[dict[str, jnp.ndarray]],
    targets: list[dict[str, jnp.ndarray]],
) -> dict[str, float | int]:
    """Evaluate pipeline on a dataset.

    Args:
        pipeline: Trained pipeline module.
...
def weighted_cross_entropy_loss(
    ...
    Args:
        logits: Model logit predictions.
...
def main() -> None:
    """Run the variant calling training pipeline."""
    rng = np.random.default_rng(42)
    _ = rng  # seed used via JAX keys below
    ...
    for epoch in range(100):
        ...
        for inp, tgt in zip(train_inputs, train_targets):
            ...
            def compute_loss(model: nnx.Module) -> jnp.ndarray:
